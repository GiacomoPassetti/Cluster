...
def full_sweep(psi, step, U):
  for _ in range(step):
   for i in range((L-2)//2):
     
     apply_local_cav_r(psi, 2*i, U[2*i]) 
     apply_local_cav_r(psi, 2*i+1, Id)  
   
   apply_local_cav_end(psi, L-2, U[L-2])
   for i in range((L-2)//2):
...
def GS_search(psi, H_bond, L):
    for dt in [0.1,1.e-2,1.e-3,1.e-4, 1.e-5]:
     U=[]
     for i in range(L-1):
        U.append(U_bond(dt, H_bond[i]))
     full_sweep(psi, 100, U)
