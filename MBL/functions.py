...
def sites(L):
 FSite=SpinSite(S=0.5, conserve='Sz')
...
def H_vector(psi, J, h1, h2, L):
    ...
    sxx=npc.outer((J/2)*psi.sites[0].Sp.replace_labels(['p', 'p*'], ['p0', 'p0*']),psi.sites[1].Sm.replace_labels(['p', 'p*'], ['p1', 'p1*'])).itranspose([0,2,1,3])
    syy=npc.outer((J/2)*psi.sites[0].Sm.replace_labels(['p', 'p*'], ['p0', 'p0*']),psi.sites[1].Sp.replace_labels(['p', 'p*'], ['p1', 'p1*'])).itranspose([0,2,1,3])
...
def H_bonds_random(psi, J, L, hmax):
    randi=[]
    for i in range(L):
        randi.append((random.random()*2*hmax)-(hmax))
    H_B=[None]
    H_B.append(H_vector(psi, J, randi[0], randi[1]/2, L))
    for i in range(L-3):
        H_B.append(H_vector(psi, J, randi[i+1], randi[i+2]/2, L))
    H_B.append(H_vector(psi, J, randi[L-2]/2, randi[L-1], L))
    return H_B
    
def H_bonds_random_randi(psi, J, L, hmax, randi):
    
    H_B=[None]
    H_B.append(H_vector(psi, J, 2*randi[0]*hmax-hmax, (randi[1]*hmax/2)-hmax/2, L))
    for i in range(L-3):
        H_B.append(H_vector(psi, J, randi[i+1], randi[i+2]/2, L))
