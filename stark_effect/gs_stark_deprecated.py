...
def H_Peier(g, J, Omega, hh, U):
  ...
  for i in range(int(L/2)):
    ...
    ons_l=npc.outer(psi.sites[0].Id.replace_labels(['p','p*'],['p0', 'p0*']),npc.outer((h*(L-2*i-1))*psi.sites[1].N.replace_labels(['p', 'p*'], ['p1', 'p1*']),psi.sites[1].Id.replace_labels(['p', 'p*'], ['p2', 'p2*'])) ).itranspose([0,2,4,1,3,5])
    ons_r=npc.outer(psi.sites[0].Id.replace_labels(['p','p*'],['p0', 'p0*']),npc.outer(psi.sites[1].Id.replace_labels(['p', 'p*'], ['p1', 'p1*']),(h*(L-2*i))*psi.sites[1].N.replace_labels(['p', 'p*'], ['p2', 'p2*'])) ).itranspose([0,2,4,1,3,5])
...
NN_b=[]
g_s=[]
for g in list(np.arange(0,0.55, 0.05)):
...
np.save('Avg_bos_stark'+ID, N_b)
np.save('Ph_sq_stark'+ID, NN_b)
