...
def diff_termo(L, g):
    ...
    H2=XX*((g**2)/(2*np.pi))+(Omega*Nb)
    w, v= eigsh(H2, 1, which='SA')
    v=v[:,0]
    N_ph_th=expectation(v, Nb)
    diff=N_ph_th-N_ph_all
...
plt.figure(dpi=800)
plt.loglog(Ls, Y,"black", Ls, Y2, Ls, Y3, Ls, Y4, Ls, Y5,  Ls, Y6)
plt.xlabel("L")
plt.ylabel(r"$<a^{\dag}a>_{2nd}-<a^{\dag}a>_{Exact}$")
plt.legend(["1/L",  "g=0.2", "g=0.5", "g=1", "g=2", "g=5"])
