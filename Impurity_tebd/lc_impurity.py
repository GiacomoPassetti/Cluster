...
def Suz_trot_real_imp(psi, dt, N_steps, H_bond_tebd):
    trunc_err=tenpy.algorithms.truncation.TruncationError(eps=0.0, ov=1.0)
    U_ev=[U_bond(1j*dt, H_bond_tebd)]*(int(L/2)-1)
    U_odd= [U_bond((1j*dt)/2, H_bond_tebd)]*(int(L/2)-1)
    U_ev[-int(L/4)]=U_bond(1j*dt, imp_ev)
    U_odd[int(L/4)-1]=U_bond((1j*dt)/2, imp_odd)
...
def LC(psi, tmax):
  ...
  for i in range(int(tmax/dt)):
     ...
     eps=eps + Suz_trot_real_imp(psi, dt, N_steps, H_bond_tebd).eps
     n_i=[]
     for j in range(L):
...
with open('C:/Users/giaco/Desktop/Cluster/Impurity_tebd/GS_J_1V_0L_20DMRG.pkl', 'rb') as f:
...
imp_ev=H_Peier_imp(g, J, Omega, h)[0]
imp_odd=H_Peier_imp(g, J, Omega, h)[1]
